...
from app.repositories.memory import CLIENT_IDS, DEMO_FIRM_ID  # noqa: E402
...
async def seed() -> None:
    ...
    if settings.use_in_memory_db:
        ...
        print("Set USE_IN_MEMORY_DB=false and Supabase credentials to seed hosted Supabase.")
        return
    store = SupabaseStore(settings)
    client = store.client
    users = {
        "firm_admin": _ensure_user(client, email=settings.demo_admin_email, password=settings.demo_admin_password, full_name="Ananya Sharma"),
        "gst_preparer": _ensure_user(client, email=settings.demo_preparer_email, password=settings.demo_preparer_password, full_name="Aman Verma"),
        "reviewer": _ensure_user(client, email=settings.demo_reviewer_email, password=settings.demo_reviewer_password, full_name="Priya Nair"),
    }
    client.table("firms").upsert({"id": DEMO_FIRM_ID, "name": "Sharma & Associates", "slug": "sharma-associates"}).execute()
    profile_rows = [
        {"id": users["firm_admin"], "full_name": "Ananya Sharma", "email": settings.demo_admin_email},
        {"id": users["gst_preparer"], "full_name": "Aman Verma", "email": settings.demo_preparer_email},
        {"id": users["reviewer"], "full_name": "Priya Nair", "email": settings.demo_reviewer_email},
    ]
    ...
    client.table("firm_members").upsert(membership_rows, on_conflict="firm_id,user_id").execute()

    for client_id, business, legal, gstin, state, kind, frequency, contact, phone, scenario in CLIENTS:
    ...
    for app_id, client_id, label, status in APPS:
        ...
        current = client.table("document_requirements").select("id").eq("application_id", app_id).execute().data or []
        if not current:
            rows = []
            for requirement_type, display in REQUIREMENTS.items():
                rows.append({
                    ...
                    "label": display,
                    "required": True,
                    "status": "missing" if client_id == CLIENT_IDS["raj"] else "received",
                })
    ...
    print("Seeded Sharma & Associates, three users, five clients, five GST applications, and demo knowledge.")
