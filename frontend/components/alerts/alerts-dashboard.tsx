...
export function AlertsDashboard() {
  ...
  return <div className="grid gap-6">
    <Card className="p-5"><div className="flex flex-wrap items-center justify-between gap-4">
      ...
      <div className="flex rounded-xl border border-[var(--obliq-border)] bg-[var(--obliq-soft)] p-1">{(["all", "validation", "reconciliation"] as AlertScope[]).map(value => <button key={value} onClick={() => setScope(value)} className={`rounded-lg px-3 py-2 text-xs font-semibold ${scope === value ? "bg-[var(--obliq-surface)] shadow-sm" : "text-[var(--obliq-muted)]"}`}>{value === "all" ? "All" : formatStatus(value)}</button>)}</div>
    </div></Card>
    <Card className="overflow-hidden"><div className="border-b border-[var(--obliq-border)] p-5"><h3 className="font-bold">Alerts Dashboard</h3></div><div className="divide-y divide-[var(--obliq-border)]">
      {visibleAlerts.map(alert => <button key={alert.id} onClick={() => setSelected(alert)} className="grid w-full gap-3 p-5 text-left hover:bg-[var(--obliq-soft)] sm:grid-cols-[1fr_.7fr_.7fr_auto]">
