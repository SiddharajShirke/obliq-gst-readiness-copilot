...
import {AlertTriangle, CheckCircle2, FileUp, GitCompareArrows, Play} from "lucide-react";
...
export function ReconciliationPanel({applicationId, onChanged}: {applicationId: string; onChanged: () => void}) {
  ...
  async function itemAction(item: ReconciliationItem, kind: "review" | "raise-alert") {
    ...
    try {
      ...
      toast.success(kind === "review" ? "Finding marked reviewed" : "Alert raised for CA follow-up"); await load();
