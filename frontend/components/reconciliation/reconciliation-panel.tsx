...
export function ReconciliationPanel({applicationId, onChanged, onGstr2bStatusChange}: {
  applicationId: string;
  onChanged: () => void;
  onGstr2bStatusChange?: (status: string) => void;
}) {
  ...
  const load = useCallback(async () => {
    ...
    setResult(next); setGstr(status); onGstr2bStatusChange?.(status.status);
  }, [applicationId, onGstr2bStatusChange]);
  useEffect(() => {
