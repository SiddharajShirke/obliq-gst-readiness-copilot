...
import { BellRing, BriefcaseBusiness, LayoutDashboard, LogOut, Menu, Settings, Users, X } from "lucide-react";
...
const links = [
  ...
  { href: "/dashboard/gst-work", label: "GST Work", icon: BriefcaseBusiness },
...
function SidebarContent({ close }: { close?: () => void }) {
  ...
  return <div className="flex h-full flex-col">
    ...
    <nav className="grid gap-1 p-3">{links.map(({href,label,icon:Icon})=>{const active=href==="/dashboard"?pathname==="/dashboard":href==="/dashboard/gst-work"?pathname.startsWith("/dashboard/gst-work")||pathname.startsWith("/dashboard/applications"):pathname.startsWith(href);return <Link key={label} onClick={close} href={href} aria-current={active?"page":undefined} className={`obliq-focus flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active?"obliq-nav-selected":"obliq-interactive"}`}><Icon size={18}/>{label}</Link>})}</nav>
