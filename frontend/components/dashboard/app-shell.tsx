...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-[#f8f7f5] lg:grid lg:grid-cols-[248px_1fr]">
    ...
    <div className="lg:col-start-2"><header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#e5e2de] bg-[#f8f7f5]/90 px-4 backdrop-blur-lg lg:px-8"><button onClick={()=>setOpen(true)} className="rounded-full p-2 lg:hidden" aria-label="Open menu"><Menu size={21}/></button><div className="hidden text-sm text-[#77716e] lg:block">Sharma & Associates · GST Readiness Workspace</div><span className="rounded-full border border-[#dcd7d2] bg-white px-4 py-2 text-xs font-semibold">Vonage Sandbox</span></header><main className="mx-auto max-w-[1500px] p-4 sm:p-6 lg:p-8">{children}</main></div>
  </div>;
}
